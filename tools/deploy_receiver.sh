#!/bin/bash
# Deploy Receiver Application to ESP32
# Usage: ./deploy_receiver.sh PORT
# Example: ./deploy_receiver.sh COM3
# Example (Linux/Mac): ./deploy_receiver.sh /dev/ttyUSB0

if [ -z "$1" ]; then
    echo "Usage: deploy_receiver.sh PORT"
    echo "Example (Windows): ./deploy_receiver.sh COM3"
    echo "Example (Linux/Mac): ./deploy_receiver.sh /dev/ttyUSB0"
    exit 1
fi
